...
export default function QAChecklist() {
  ...
  const deriveBaseName = (name) => {
    ...
    if (parts.length >= 5) {
      ...
      const middle = parts.slice(2, -1).filter((p) => !/^[A-Z]{2,4}$/.test(p));
      return [first, second, ...middle, last].join("_");
    }
    return name;
  };

  const deriveMeshName = (name) => "SM_" + deriveBaseName(name);
  const deriveMaterialName = (name) => "M_" + deriveBaseName(name);
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .naming-tag.xform { background: rgba(42,107,156,0.25); color: #6aadde; }
        .naming-tag.mesh  { background: rgba(27,122,90,0.25);  color: #4eca7a; }
        .naming-tag.looks { background: rgba(139,74,43,0.25);  color: #d4855a; }
        .naming-tag.mat   { background: rgba(120,60,140,0.25); color: #c07ad4; }
        ...
        .naming-status.ok  { color: var(--pass); }
        .naming-status.err { color: var(--fail); }
        ...
        <div className="qa-header">
          ...
          {assetName && (
            <div className="naming-preview">
              <div className="naming-preview-title">命名層級預覽</div>
              <div className="naming-node">
                <span className="naming-connector"> </span>
                <span className="naming-tag xform">Xform</span>
                <span className="naming-value">{assetName}</span>
                {isValidXformName(assetName)
                  ? <span className="naming-status ok">✓ 駝峰式</span>
                  : <span className="naming-status err">✕ 非駝峰式</span>}
              </div>
              <div className="naming-node" style={{ paddingLeft: 14 }}>
                <span className="naming-connector">├</span>
                <span className="naming-tag mesh">Mesh</span>
                <span className="naming-value">{deriveMeshName(assetName)}</span>
              </div>
              <div className="naming-node" style={{ paddingLeft: 14 }}>
                <span className="naming-connector">└</span>
                <span className="naming-tag looks">Looks</span>
              </div>
              <div className="naming-node" style={{ paddingLeft: 28 }}>
                <span className="naming-connector">└</span>
                <span className="naming-tag mat">Mat</span>
                <span className="naming-value">{deriveMaterialName(assetName)}</span>
                {isValidXformName(deriveMaterialName(assetName).slice(2))
                  ? <span className="naming-status ok">✓ 駝峰式</span>
                  : <span className="naming-status err">✕ 非駝峰式</span>}
              </div>
            </div>
